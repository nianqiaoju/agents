...
boarding_csmc_sparse_fast <- function(y, N, alpha0, lambda, gamma, rho, neighbors, logpolicy, num_particles = 20, ess_threshold = 0.5){
  ...
  stitindex <- sample.int(n = dim(lowdimstates)[1], size = num_particles, replace = TRUE, prob = normalizedposterior);
  ...
  for (t in 1 : (num_observations - 1)){
    ...
    if(ess < ess_threshold | any(is.infinite(logweights))){
      ancester <- sample.int(n = num_particles, prob = weights, replace = TRUE);
