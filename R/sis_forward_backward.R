...
sis_forward_backward <- function(y, model_config, num_samples = 1, all_xx = NULL){
	if(model_config$N >= 5) stop('population too large for forward backward');
	if(is.null(all_xx)) all_xx <- sis_get_state_space(model_config$N);
	num_observations <- length(y);
	## f0 density
	logf0 <- rep(NA, dim(all_xx)[1])
	logf0 <- apply(all_xx, 1, function(xx) logdbern_sum_cpp(model_config$alpha0, xx))
	## log f(x(t) | x(t-1))
	all_alpha <- t(apply(all_xx, 1, function(xx) sis_get_alpha(xx, model_config)))
	logdtransition <- matrix(NA, nrow = dim(all_xx)[1], ncol = dim(all_xx)[1])
	for(istate in 1 : dim(all_xx)[1]){
		logdtransition[istate,] <- apply(all_xx, 1, function(xx) logdbern_sum_cpp(all_alpha[istate,], xx))
	}
	all_sum_x <- rowSums(all_xx)
	...
	if (num_samples >= 1){
		...
		for( t in (num_observations - 2) : 0){
			bs_indices[, t + 1] <- sapply(bs_indices[, t + 1 + 1], 
				function(istate) sample.int(n = dim(all_xx)[1], size = 1, prob = lw.normalize(logdtransition[, istate] + M[, t + 1])));
		}
