...
load("~/Dropbox/AgentBasedModels/agents/figures/section3/data_generate_grid_hetero_slopes.RData");
...
##
cat("[reminder: data generating parameters are: bg2 = ", dgp_bg2,",bl2 = ",dgp_bl2,"]\n");
...
mle_summary <- data.frame(llik_df %>% group_by(t) %>% summarise(maxllik = max(llik), bg2 = bg2[which(llik == maxllik)], bl2 = bl2[which(llik == maxllik)]));
...
setmytheme();
# g <- ggplot(data = llik_df, aes(x = bg2, y = bl2));
...
heatmap <- ggplot(data = llik_df, aes(x = bl2, y = bg2, group = t));
heatmap <- heatmap + facet_grid( . ~ t)+ geom_tile(aes(fill = llik)) ;
heatmap <- heatmap + geom_point(aes(x = dgp_bl2, y = dgp_bg2), colour = "black", size = 4) + 
  geom_point(data = mle_summary, aes(x = bl2, y = bg2, group = t), colour = "red", size = 4);
heatmap <- heatmap + scale_fill_gradientn(colours=c("white", cbPalette[8], cbPalette[3], "darkgreen", cbPalette[5]),
                                          ...
                                          guide="colorbar")
heatmap <- heatmap + guides(fill = guide_colorbar(title = "log-likelihood", barwidth = 40));
heatmap <- heatmap + xlab(expression(beta[lambda]^2)) + ylab(expression(beta[gamma]^2))
heatmap

ggsave(filename = "~/Dropbox/AgentBasedModels/paper/sis.likelihood.heatmap.hetero.slopes.pdf", plot = heatmap,
