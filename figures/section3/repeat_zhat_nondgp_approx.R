...
approx_llik_df_notdgp <- data.frame(method = factor(character(df_length), levels = c("csmcApprox","apfApprox")),
...
for(p in num_particles_list){
  ...
  for(irep in 1 : num_repeats){
    result_ <- sis_csmc(y, new_config, approx_config);
    approx_llik_df_notdgp[irow,1] <- "csmcApprox";
    approx_llik_df_notdgp[irow,2] <- p;
    approx_llik_df_notdgp[irow,3] <- irep;
    approx_llik_df_notdgp[irow,4] <- result_$log_final_likelihood;
    approx_llik_df_notdgp[irow,5] <- result_$totaltime;
    irow <- irow + 1;
    cat("csmc repetition:", irep, "/", num_repeats, "particles:", p, "\n")
    
    result_ <- sis_apf(y, new_config, approx_config);
    approx_llik_df_notdgp[irow,1] <- "apfApprox";
    approx_llik_df_notdgp[irow,2] <- p;
    approx_llik_df_notdgp[irow,3] <- irep;
    approx_llik_df_notdgp[irow,4] <- result_$log_final_likelihood;
    approx_llik_df_notdgp[irow,5] <- result_$totaltime;
  ...
  save.image("figures/section3/data_repeat_zhat_nondgp_approx.RData")
}

# head(approx_llik_df_notdgp);
# tail(approx_llik_df_notdgp);
