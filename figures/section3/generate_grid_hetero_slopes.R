...
### create a grid for beta_gamma_2 & beta_lambda_2 and check for identifiablity
bg2_grid <- bl2_grid <- seq(-6,6,length.out = 30);
...
df_length <- length(time_grid) * length(bg2_grid) * length(bl2_grid);
llik_df <- data.frame(t = integer(df_length), 
                      bg2 = double(df_length), 
...
for(bg2 in bg2_grid){
  for(bl2 in bl2_grid){
    ...
    model_config$lambda <- get_lambda(b1 = dgp_bl1, b2 = bl2);
    model_config$gamma <- get_lambda(b1 = dgp_bg1, b2 = bg2);
...
save.image("figures/section3/data_generate_grid_hetero_slopes.RData")